@extends('facture.layout')

@section('body')
<a href="#" class="dropdown-item" data-action="app-print">
    <span data-i18n="drpdwn.print"><button id="print" class="btn btn-primary">Imprimer (Click me !!!)</button></span>
    <i class="float-right text-muted fw-n">Ctrl + P</i>
</a>
@foreach($lodges as $lodge)
<main id="js-page-content" role="main" class="page-content">
                        <ol class="breadcrumb page-breadcrumb">
                            <li class="breadcrumb-item"><a href="javascript:void(0);">E-Gelodge</a></li>
                           
                            <li class="breadcrumb-item active">Contrat</li>
                            <li class="position-absolute pos-top pos-right d-none d-sm-block"><span class="js-get-date"></span></li>
                        </ol>
                        
                        <div class="container">
                            <div data-size="A4">
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="d-flex align-items-center mb-5">
                                            <h2 class="keep-print-font fw-500 mb-0 text-primary flex-1 position-relative">
                                                E-Gelodge WebApp
                                                <small class="text-muted mb-0 fs-xs">
                                                  
                                                </small>
                                                <!-- barcode demo only -->
                                                <img id="barcode" alt="" class="position-absolute pos-top pos-right height-3 mt-1 hidden-md-down" src="data:image/png;base64,">
                                            </h2>
                                        </div>
                                    
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4 d-flex">
                                        <div class="table-responsive">
                                            <table class="table table-clean table-sm align-self-end">
                                                <tbody>
                                                    
                                                    <tr>
                                                        <td>
                                                            Issue Date:
                                                        </td>
                                                        <td>
                                                            {{$lodge->updated_at}}
                                                        </td>
                                                    </tr>

                                                    <tr>
                                                        <td>
                                                            <strong>Due Date:</strong>
                                                        </td>
                                                        <td>
                                                            {{$lodge->updated_at}}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>
                                                            Net:
                                                        </td>
                                                        <td>
                                                            {{$lodge->prix}}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>
                                                            Currency:
                                                        </td>
                                                        <td>
                                                            Ar
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>
                                                            Concerné:
                                                        </td>
                                                        <td>
                                                            {{$lodge->client->name}}
                                                        </td>
                                                    </tr> 
                                                    <tr>
                                                        <td>
                                                            Adresse:
                                                        </td>
                                                        <td>
                                                             {{$lodge->client->adress}}
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>
                                                            CIN:
                                                        </td>
                                                        <td>
                                                           {{$lodge->client->cin}}
                                                        </td>
                                                    </tr>

                                                  
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                    <div class="col-sm-4 ml-sm-auto">
                                        <div class="table-responsive">
                                            <table class="table table-sm table-clean text-right">
                                                <tbody>
                                                  
                                                    <tr>
                                                        <td>
                                                            <strong>Compagnie LODGE</strong>
                                                        </td>
                                                    </tr>
                                                    
                                                    <tr>
                                                        <td>
                                                            Tanarive 101 , Rue RADAMA I
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>
                                                            +261 34 750 06 42
                                                        </td>
                                                    </tr>
                                                    <tr>
                                                        <td>
                                                            lodgecompagnie@gmail.com
                                                        </td>
                                                    </tr>
                                                  
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                                <?php 
                                    $tranche=($lodge->prix)/($lodge->contrat->duration);
                                ?>
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="table-responsive">
                                            <table class="table mt-5">
                                                <thead>
                                                    <tr>
                                                        <th class="text-center border-top-0 table-scale-border-bottom fw-700">*</th>
                                                        <th class="border-top-0 table-scale-border-bottom fw-700">Item</th>
                                                        <th class="border-top-0 table-scale-border-bottom fw-700">Contrat</th>
                                                    
                                                        <th class="text-center border-top-0 table-scale-border-bottom fw-700">Duré d'echeance</th>
                                                        <th class="text-right border-top-0 table-scale-border-bottom fw-700">Valeur echeante</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    <tr>
                                                        <td class="text-center fw-700">1</td>
                                                        <td class="text-left strong">tranche</td>
                                                        <td class="text-left">{{$lodge->contrat->libele}}</td>
                                                        
                                                        <td class="text-center">{{$lodge->contrat->duration}} mois</td>
                                                        <td class="text-right">{{$tranche}}</td>
                                                    </tr>

                                                   
                                                    
                                                </tbody>
                                            </table>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
                                    <div class="col-sm-4 ml-sm-auto">
                                        <table class="table table-clean">
                                            <tbody>
                                                <tr>
                                                    <td class="text-left">
                                                        <strong>Subtotal</strong>
                                                    </td>
                                                    <td class="text-right">{{$lodge->prix}}</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-left">
                                                        <strong>Discount (0%)</strong>
                                                    </td>
                                                    <td class="text-right">0</td>
                                                </tr>
                                                <tr>
                                                    <td class="text-left">
                                                        <strong>TVA (0%)</strong>
                                                    </td>
                                                    <td class="text-right">0</td>
                                                </tr>
                                                <tr class="table-scale-border-top border-left-0 border-right-0 border-bottom-0">
                                                    <td class="text-left keep-print-font">
                                                        <h4 class="m-0 fw-700 h2 keep-print-font color-primary-700">Total</h4>
                                                    </td>
                                                    <td class="text-right keep-print-font">
                                                        <h4 class="m-0 fw-700 h2 keep-print-font">{{$lodge->prix}}</h4>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="text-left">
                                                        <strong>Paid</strong>
                                                    </td>
                                                    <td class="text-right">
                                                        <strong>0</strong>
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td class="text-left keep-print-font">
                                                        <h4 class="m-0 fw-700 h2 keep-print-font color-primary-700">Amount Due</h4>
                                                    </td>
                                                    <td class="text-right keep-print-font">
                                                        <h4 class="m-0 fw-700 h2 keep-print-font text-danger">{{$lodge->prix}}</h4>
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                <div class="row">
                                    <?php
                                        $reste=($lodge->contrat->duration)-($lodge->compte);

                                    ?>
                                    <div class="col-sm-12">
                                       <h4 class="py-4 text-primary">
                                            Contrat : {{$lodge->contrat->libele}} <br>
                                            Durée d'echeance: {{$lodge->contrat->duration}} mois <br>
                                            Durée Echeance accomplie:@if(is_null($lodge->compte)) 0 @else {{$lodge->compte}} @endif mois <br>
                                            Durée Echeance restante:{{$reste}} mois
                                        </h4>
                                        <h4 class="py-2 text-primary">
                                            Je déclare sur l'honneur porté le nom {{$lodge->client->name}} au-dessus que j'accepte tous les conditions du payement.
                                            <br>
                                            Dans le cas de non respect de contrat , j'accepte les sanctions prises par Lodge Compagnie.
                                        </h4>
                                        <p class="mt-2 text-muted mb-0">
                                            Payment details: ACC:123006705 IBAN:US100000060345 SWIFT:BOA447
                                        </p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </main>
            @endforeach


@endsection